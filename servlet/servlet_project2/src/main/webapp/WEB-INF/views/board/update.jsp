...
<form action="<c:url value="/board/update"/>" method="post" enctype="multipart/form-data">
	...
	<div class="mb-3 mt-3">
		...
		<textarea class="form-control" rows="10" id="content" name="content" placeholder="내용">${board.bo_content}</textarea>
	</div>
	<div class="mb-3 mt-3" id="attachment">
		<label for="file" class="form-label">첨부파일:</label>
		<!-- 첨부파일을 출력할 forEach -->
		<c:forEach items="${fileList}" var="file">
			<span class="form-control">${file.fi_ori_name} <span class="btn-del" style="cursor: pointer;" data-target="${file.fi_num}">&times;</span></span>
		</c:forEach>
		<!-- 남은 첨부파일 자리에 출력할 input 태그 forEach -->
