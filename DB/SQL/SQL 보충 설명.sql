...
SELECT CA_NAME AS 카테고리, FLOOR(AVG(PR_PRICE)) AS 가격평균
FROM PRODUCT JOIN CATEGORY ON CA_NUM = PR_CA_NUM
GROUP BY PR_CA_NUM;

# 각 카테고리별 제품 가격 평균이 6500원 이상인 카테고리를 조회
SELECT * FROM
(SELECT CA_NAME AS 카테고리, FLOOR(AVG(PR_PRICE)) AS 가격평균
FROM PRODUCT JOIN CATEGORY ON CA_NUM = PR_CA_NUM GROUP BY PR_CA_NUM) AS AVG_TMP
HAVING 가격평균 >= 6500;
