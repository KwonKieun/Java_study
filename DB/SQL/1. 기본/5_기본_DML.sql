...
INSERT INTO `MEMBER`(PW, ID, REG_DATE, EMAIL)
	VALUES("비번132", "ID1", NOW(), "ID1@naver.com");    
# 속성명을 생략한 대신, 테이블에 등록된 컬럼 순서대로 값들을 넣어주어야 함
INSERT INTO MEMBER
	VALUES("ID2", "비번입니다.", "ABCDEF@GAMIL.COM", NOW());
# NOW()를 이용해서 날짜를 문자열에 저장하면 날짜가 문자열로 변환돼서 문제가 없음
# 날짜형태가 아닌 문자열을 DATETIME에 저장하려 하면 에러 발생
-- INSERT INTO MEMBER
-- 	VALUES("ID3", "비번입니다.", NOW(), "ABCDEF@GAMIL.COM");
# 날짜형태인 문자열을 DATATIME에 저장하려 하면 변환이 가능하기 때문에 정상 동작
INSERT INTO MEMBER
...
UPDATE
	MEMBER
SET
	PW = "IDABC", EMAIL = "IDABC@NAVER.COM"
WHERE
	ID = "ID1";
