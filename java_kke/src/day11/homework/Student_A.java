...
public class Student_A {
	...
	public Subject_A[] getSubjects() {
		return subjects;
	}

	public void setSubjects(Subject_A[] subjects) {
	...
	public boolean insertSubject(String title, int score) {
		...
		//과목을 등록
		subjects[count] = new Subject_A(title, score);
		count++;
		expandSubject();
		return true;
	}
	private void expandSubject() {
		...
		//차서 늘려줌
		Subject_A [] tmpList = new Subject_A[subjects.length + 5];
	...
	public boolean deleteSubject(String title) {
		...
		Subject_A [] tmpList = new Subject_A[subjects.length];
		//기존 과목 전체복사
		System.arraycopy(subjects, 0, tmpList, 0, subjects.length);
		//앞으로 당겨주는 작업
	...
	public Student_A(int grade, int classNum, int num, String name) {
		this.name = name;
		this.grade = grade;
		this.classNum = classNum;
		this.num = num;
