...
public class BoardMain {
	...
	private static int boardNum = 1; //추가된 게시글 번호
	...
	private static void printBoardDetail() {
		...
		for(int i = 0; i < count; i++) {
			...
			if(boardList[i].getNum() == num) {
				// 조회수 ++
				int views = boardList[i].getViews();
				boardList[i].setViews(++views);
	...
	private static void insertBoard() {
		...
		System.out.print("제목 : ");
		String title = scan.nextLine(); // 공백있음
		System.out.print("내용 : ");
		String contents = scan.nextLine();
		System.out.print("일자 : ");
		String date = scan.next(); // 공백없이
